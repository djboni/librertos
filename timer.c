...
static void _OS_timerInsertInOrderedList(struct Timer_t *timer,
                                         tick_t tickToWakeup) {
  ...
  for (;;) {
    ...
    if (pos != (struct taskListNode_t *)list && pos->List != list &&
...
static void _OS_timerFunction(taskParameter_t param) {
  ...
  if (changeIndex != 0) {
    ...
    OSstate.TaskTimerLastRun = 0;
             ...
             (OSstate.TimerIndex ==
                  (struct taskListNode_t *)&OSstate.TimerList ||
    ...
    if (OSstate.TimerIndex != (struct taskListNode_t *)&OSstate.TimerList) {
      ...
      INTERRUPTS_ENABLE();
    } else if (OSstate.TimerList.Head !=
               (struct taskListNode_t *)&OSstate.TimerList) {
...
void Timer_reset(struct Timer_t *timer) {
  ...
  OS_listInsertAfter(&OSstate.TimerUnorderedList,
