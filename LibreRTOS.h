...
enum timerType_t {
  TIMERTYPE_ONESHOT =
      0x00, /* Timer need to be reset to run. Run after period has passed. */
  TIMERTYPE_AUTO = 0x01, /* Auto reset timer after it has run. */
  TIMERTYPE_NOPERIOD =
      0x02 /* Timer need to be reset to run. Run as soon as it is reset. */
};
...
struct libreRtosState_t {
  ...
  volatile schedulerLock_t SchedulerLock; /* Scheduler lock. Controls if another
                                             task can be scheduled. */
  volatile schedulerLock_t SchedulerUnlockTodo; /* Flags the schedule unlock
  ...
  struct taskHeadList_t
      *BlockedTaskList_NotOverflowed; /* List with blocked tasks (not
                                         overflowed). */
  struct taskHeadList_t
      *BlockedTaskList_Overflowed; /* List with blocked tasks (overflowed). */

  struct taskHeadList_t
      PendingReadyTaskList; /* List with ready tasks not removed from list of
                               blocked tasks. */
  struct taskHeadList_t
      BlockedTaskList1; /* List with blocked tasks number 1. */
  struct taskHeadList_t
      BlockedTaskList2; /* List with blocked tasks number 2. */

#if (LIBRERTOS_SOFTWARETIMERS != 0)
  tick_t TaskTimerLastRun;
  struct task_t TaskTimerTCB; /* Task control block of timer task. */
  struct taskListNode_t
      *TimerIndex; /* Points to next timer to be run in TimerList. */
  struct taskHeadList_t
      TimerList; /* List of running timers ordered by wakeup time. */
  struct taskHeadList_t TimerUnorderedList; /* List of running timers to be
...
void OS_taskCreate(struct task_t *task, priority_t priority,
                   taskFunction_t function, taskParameter_t parameter);
...
void Timer_init(struct Timer_t *timer, enum timerType_t type, tick_t period,
                timerFunction_t function, timerParameter_t parameter);
