...
static void _OS_schedulerReal(void) {
  ...
  for (;;) {
    ...
    {
      /* Scheduler locked. We can read CurrentTCB directly. */
      priority_t currentTaskPriority =
          (OSstate.CurrentTCB == NULL ? LIBRERTOS_NO_TASK_RUNNING
                                      : OSstate.CurrentTCB->Priority);
      priority_t priority;

      for (priority = LIBRERTOS_MAX_PRIORITY - 1;
           priority > currentTaskPriority; --priority) {
...
/** Lock scheduler (recursive lock). Current task cannot be preempted if
 scheduler is locked. */
void OS_schedulerLock(void) { ++OSstate.SchedulerLock; }
...
static void _OS_tickUnblockTimedoutTasks(void) {
  ...
  while (OSstate.BlockedTaskList_NotOverflowed->Length != 0 &&
    ...
    struct task_t *task =
        (struct task_t *)OSstate.BlockedTaskList_NotOverflowed->Head->Owner;
...
static void _OS_tickUnblockPendingReadyTasks(void) {
  ...
  while (OSstate.PendingReadyTaskList.Length != 0) {
    struct task_t *task =
        (struct task_t *)OSstate.PendingReadyTaskList.Head->Owner;
...
void OS_taskResume(struct task_t *task) {
  ...
  OS_schedulerLock();
  {
    CRITICAL_ENTER();
    {
      ...
      OS_listInsertAfter(&OSstate.PendingReadyTaskList,
                         OSstate.PendingReadyTaskList.Head, node);
...
void OS_eventRInit(struct eventR_t *o) { OS_listHeadInit(&o->ListRead); }
...
void OS_eventPendTask(struct taskHeadList_t *list, struct task_t *task,
   ...
   be changed by interrupts, so we must do things carefully. */
  {
    ...
    for (;;) {
      ...
      if (pos != LIST_HEAD(list) && pos->List != list && node->List == list) {
